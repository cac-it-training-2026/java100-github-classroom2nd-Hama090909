...
class Omegalian {
	...
	public void setItem(String item) {
		...
		char last = item.charAt(lastIndex);
		String changeStr = changeLast(last);

		if (changeStr != null) {
			StringBuffer st = new StringBuffer(item);
			st.replace(lastIndex, lastIndex + 1, changeStr);
			this.item = new String(st);
		} else {
			this.item = item;
		}
	}

	private String changeLast(char ch) {
