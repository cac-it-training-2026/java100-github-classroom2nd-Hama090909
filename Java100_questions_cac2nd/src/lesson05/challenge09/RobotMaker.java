...
class Robot {
	...
	void makeOmelet(int eggNum, int butterNum) {
		int omlet1 = eggNum / 2;
		int omlet2 = butterNum / 5;

		if (omlet1 > omlet2) {
			System.out.println(omlet2 + "人分のオムレツを作成しました。");
		} else {
			System.out.println(omlet1 + "人分のオムレツを作成しました。");
		}
	}
	...
	String makeEggDishes(int flourNum, int sugarNum, int eggNum, int butterNum) {
		...
		String dish = null;
		if (flour >= 170 && sugar >= 50 && egg >= 1 && butter >= 5) {
			dish = "クッキー";
...
public class RobotMaker {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(dish + "ができました");
